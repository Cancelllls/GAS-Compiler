...
function transpileJava(code) {
  ...
  class LinkedBlockingQueue {
    ...
    put(item) { this.queue.push(item); }
    poll(timeout, timeUnit) { return this.queue.shift() || null; }
  }
  ...
  class MockExecutorService {
    ...
    shutdown() {}
    awaitTermination(timeout, unit) { return true; }
  }
  const Executors = {
...
function transpileJS(code) {
  ...
  // Inject Node.js event emitter and timer mocks
  const mocks = `
  // Node.js events.EventEmitter Mock & Timer mocks for GAS V8 environment
  class EventEmitter {
    constructor() {
      this._events = {};
    }
  ...
  const setInterval = function(callback, delay, ...args) {
    const start = Date.now();
    while (Date.now() - start < delay) {}
    if (callback) callback(...args);
    return 1;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Comment out ES module imports: import ... from '...' or import '...'
    ...
    // Comment out exports if any: export default ... or export const ...
    if (trimmed.startsWith('export ')) {
      if (trimmed.startsWith('export default ')) {
        transpiledLine = trimmed.replace('export default ', '');
      } else {
        transpiledLine = trimmed.replace('export ', '');
      }
    }
...
function compileAndRun(filename, code, language, inputBuffer) {
  ...
  const promptMock = function(msg) {
    if (inputIdx < inputBuffer.length) {
      ...
      throw new Error("__WAITING_FOR_INPUT__");
    }
  };
  ...
  try {
    if (language === 'javascript' || language === 'gas') {
      ...
      return JSON.stringify({
        status: "success",
        logs: ["[SYSTEM] HTML/CSS rendered in Preview Pane."]
      });
    } else {
    ...
    const executor = new Function('console', 'prompt', jsCode);
    executor(consoleMock, promptMock);
